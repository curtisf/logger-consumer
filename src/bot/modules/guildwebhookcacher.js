// const { setEventLogIds } = require('../../database/interfaces/postgres')
const { setEventLogIds } = require('../../database/interfaces/lmdb')
const { memberChannelPerms } = require('../../../lib/utils/permissionCalculator')
const { setWebhook, deleteWebhook } = require('./webhookcache')
const { setGuildSettings } = require('../guildSettingsCache')

module.exports = async (guildID, channelID) => {
  const guild = await global.LoggerBot.getGuild(guildID)
  if (!guild) {
    return // TODO: do something with this
  }
  const botMember = guild.members[global.LoggerBot.user.id]
  if (!botMember) return // TODO: this is not ok, plz fix
  const botPerms = await memberChannelPerms(channelID, { ...botMember, guild_id: guildID })
  if (!botPerms['manageWebhooks']) {
    return
  }
  const logChannel = guild.channels.find(c => c.id == channelID)
  if (!logChannel) {
    console.warn(`Logchannel ${channelID} in ${global.bot.guilds.get(guildID).name} ${guildID} does not exist`)
    await deleteWebhook(channelID)
    await setEventLogIds(guildID, '') // set all event logs to empty
    await setGuildSettings(guildID)
    return
  }
  const webhooks = await global.LoggerBot.requestClient.getChannelWebhooks(channelID)
  for (let i = 0; i < webhooks.length; i++) {
    if (webhooks[i].token && webhooks[i].channel_id === channelID) { // check for token because channel subscriptions count as webhooks
      setWebhook(channelID, webhooks[i].id, webhooks[i].token)
      console.log(`G: ${guildID} C: ${channelID} found hook ${webhooks[i].id}, set cache`)
      return
    }
  }
  console.log(`Create hook > C: ${channelID} G: ${guildID} `)
  if (!logChannel) {
    console.warn(`Logchannel ${channelID} in ${guild.name} ${guildID} does not exist low`)
    await deleteWebhook(channelID)
    await setEventLogIds(guildID, '')
    await setGuildSettings(guildID)
  } else {
    const newHook = await global.LoggerBot.requestClient.createWebhook(channelID, {
      name: global.LoggerBot.user.username,
      avatar: 'data:image/jpeg;base64,'
    }, `Automated webhook creation.`).catch((e) => {
      // With permission checking, this can only trigger if Discord sends a 500 error code OR
      // the user hit the guild webhook limit. The best action is to leave because the bot
      // doesn't know who to contact when this occurs (owner? staff?)
      console.warn(`Leaving guild ${guildID} (${guild.name}, ${guild.member_count}) because of an error: ${e.message}`)
    })
    if (newHook && newHook.id) {
      console.log(`Webhook made in ${guildID} for ${channelID}`)
      await setWebhook(channelID, newHook.id, newHook.token)
      await setGuildSettings(guildID)
    } else {
      console.log(newHook)
      console.warn('Webhook "created" is invalid, please inspect')
    }
  }
}
